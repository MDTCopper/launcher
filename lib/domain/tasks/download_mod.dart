...
class DownloadJavaModTask extends Task {
  ...
  Future<void> _download() async {
    NotificationManager.addNotice(
      icon: Icons.download,
      title: '下载',
      content: '正在下载模组[${modListMeta.name}(${modMeta.name})]',
    );
    LogManager.addLog(
      LogEntry(LogType.info, '正在下载模组[${modListMeta.name}(${modMeta.name})]'),
    );

    try {
      final version = config.versionOptions.selectedVersion;
      var fileName = '${modListMeta.name}-${modMeta.name}.jar';
      ...
      var jar = modMeta.assets.where((it) => it.name.contains('.jar'));
...
class DownloadZipModTask extends Task {
  @override
  Widget buildDisplayWidget(BuildContext context) {
    // TODO: implement buildDisplayWidget
